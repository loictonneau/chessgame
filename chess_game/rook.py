...
class Rook(pieces.Piece):
    ...
    def get_available_move(self, list_pieces):  # creer une liste avec tout les coups possibles de la tours
        self.clear_available_move()
        row = self.position[0]
        column = self.position[1]

        if row < constants.row:  # ajoute les cases au dessous de la tours tant qu'aucune autre piece n'est trouvée,
            for row_down in range(row + 1, constants.row):
                piece = pieces.find_piece(list_pieces, row_down, column)
                if piece.type is None:
                    self.available_move.append([row_down, column])
        ...
        if row > 0 :  # ajoute les cases au dessus de la tours tant qu'aucune autre piece n'est trouvée
            for row_up in range(row - 1, -1, -1):
                ...
                if piece.type is None:
                    self.available_move.append([row_up, column])
                elif self.color is not piece.color:# ajoute la case si une piece adverse est trouvée puis arrete la recherche sinon arrete directement la recherche
                    self.available_move.append([row_up, column])
                    break
                elif self.color is piece.color:
        ...
        if column < constants.column:  # ajoute les cases a droite de la tours tant qu'aucune autre piece n'est trouvée
            for column_right in range(column + 1, constants.column):
                piece = pieces.find_piece(list_pieces, row, column_right)
                print (piece.position)
                if piece.type is None:
                    self.available_move.append([row, column_right])
                elif self.color is not piece.color:  # ajoute la case si une piece adverse est trouvée puis arrete la recherche sinon arrete directement la recherche
                    self.available_move.append([row, column_right])
                    break
                elif self.color is piece.color:
        ...
        if column > 0:  # ajoute les cases a gauche de la tours tant qu'aucune autre piece n'est trouvée,
            for column_left in range(column - 1, -1, -1):
                ...
                if piece.type is None:
                    self.available_move.append([row, column_left])
                elif self.color is not piece.color:  # ajoute la case si une piece adverse est trouvée puis arrete la recherche sinon arrete directement la recherche
                    self.available_move.append([row, column_left])
                    break
                elif self.color is piece.color:
